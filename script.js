  ...
  document.addEventListener('click', function(e) {
    if (e.target.matches('a[href^="#"]') || e.target.closest('a[href^="#"]')) {
      ...
      if (target) {
        ...
        const navbarHeight = document.querySelector('nav, header')?.offsetHeight || 0;
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "בית חולים זיו משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
                ...
                body: JSON.stringify({
                    websiteId: '9f43f818-5001-4712-8d12-75ba41c2d004',
                    name: data.name || '',
                    email: data.email || '',
